...
export const createTeacherProfile = async (req, res) => {
    ...
    if (user.teacherProfile) {
        res.status(409).json({ error: 'Teacher profile already exists' });
        return;
    }
...
export const updateTeacherProfile = async (req, res) => {
    ...
    const { fullName, gender, dob, address, city, state, pincode, qualification, highestQualification, instituteName, passingYear, teachingCertification, hasBEd, experience, experienceMonths, teachingMode, experienceType, experienceSummary, subjects, classes, boards, languages, availabilityStatus, } = req.body;
    const data = {};
    ...
    if (highestQualification !== undefined)
        data.highestQualification = highestQualification;
    if (instituteName !== undefined)
        data.instituteName = instituteName;
    if (passingYear !== undefined)
        data.passingYear = parseInt(passingYear);
    if (teachingCertification !== undefined)
        data.teachingCertification = teachingCertification;
    if (hasBEd !== undefined)
        data.hasBEd = Boolean(hasBEd);
    if (experience !== undefined)
        data.experience = parseInt(experience);
    if (experienceMonths !== undefined)
        data.experienceMonths = parseInt(experienceMonths);
    if (teachingMode !== undefined)
        data.teachingMode = teachingMode;
    if (experienceType !== undefined)
        data.experienceType = experienceType;
    if (experienceSummary !== undefined)
        data.experienceSummary = experienceSummary;
    if (subjects !== undefined)
        data.subjects = Array.isArray(subjects) ? subjects : [subjects];
    if (classes !== undefined)
        data.classes = Array.isArray(classes) ? classes : [classes];
    if (boards !== undefined)
        data.boards = Array.isArray(boards) ? boards : [boards];
    if (languages !== undefined)
        data.languages = Array.isArray(languages) ? languages : [languages];
...
export const getTimeSlots = async (req, res) => {
    ...
    const { day } = req.query;
    const where = { teacherId: profile.id, isActive: true };
    if (day)
        where.dayOfWeek = String(day).toUpperCase();
...
export const assignDoubt = async (req, res) => {
    ...
    if (!doubtId || !teacherId) {
        res.status(400).json({ error: 'doubtId and teacherId are required' });
        return;
    }
...
export const getTeacherDoubts = async (req, res) => {
    ...
    const where = { assignedTo: profile.id };
    if (status)
...
export const getTeacherSessions = async (req, res) => {
    ...
    const where = { teacherId: profile.id };
    if (status)
        where.status = String(status).toUpperCase();
    if (type)
...
export const createSession = async (req, res) => {
    ...
    if (!studentId || !type) {
        res.status(400).json({ error: 'studentId and type are required' });
        return;
    }
    const validTypes = ['CHAT', 'AUDIO', 'VIDEO'];
    if (!validTypes.includes(String(type).toUpperCase())) {
        res.status(400).json({ error: `type must be: ${validTypes.join(', ')}` });
        return;
    }
    const session = await prisma.session.create({
        data: { studentId, teacherId: profile.id, type: String(type).toUpperCase(), status: 'ACTIVE', startedAt: new Date() },
