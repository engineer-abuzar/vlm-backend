...
export const getMySessions = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (!studentProfile) { res.status(404).json({ message: 'Student profile not found' }); return; }
...
export const createFeedback = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  if (!rating) { res.status(400).json({ error: 'rating is required' }); return; }
  ...
  if (sessionId) {
    ...
    if (existing) { res.status(409).json({ message: 'Feedback already submitted for this session' }); return; }
  }
