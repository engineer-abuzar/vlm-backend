 ...
 */
export const getDailyMcq = async (req: AuthRequest, res: Response): Promise<void> => {
 ...
 */
export const submitMcqAttempt = async (req: AuthRequest, res: Response): Promise<void> => {
  const userId = req.user?.userId;
  if (!userId) {
    res.status(401).json({ message: 'Unauthorized' });
    return;
 ...
 */
export const createMcqQuestion = async (req: AuthRequest, res: Response): Promise<void> => {
 ...
 */
export const getAllMcqQuestions = async (_req: AuthRequest, res: Response): Promise<void> => {
  const questions = await prisma.mcqQuestion.findMany({
