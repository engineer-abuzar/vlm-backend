...
function parseDurationToDays(duration: string): number {
  ...
  if (lower.includes('quarter') || lower.includes('3 month')) return 90;
  if (lower.includes('month')) { const m = lower.match(/(\d+)/); return m ? parseInt(m[1]) * 30 : 30; }
  if (lower.includes('day')) { const d = lower.match(/(\d+)/); return d ? parseInt(d[1]) : 3; }
  return 30;
}
